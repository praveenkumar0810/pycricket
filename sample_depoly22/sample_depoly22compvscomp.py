...
for p in range(100) :
	...
	###creating directory in the name of date&time , insde the D:\py_ex\project_cricket\project_cricket_db

	datetime = datetime.datetime.now()
	dir_name =  datetime.strftime("%d-%m-%Y_%H-%M-%S-%f")

	path =  (r'D:\big_dataDB\_04\{}'.format(dir_name))
	os.mkdir(path)
	os.chdir(path)
	...
	###############function to accept only the int to score ######
	
	xaction = "On Strike = "
	yaction = "Bowl = "

	##example intOnlytoscore(xaction)
	...
	#ball_c ---> ball count     
		...
		while True :
			...
			if ball_c == ball :
				break
			usr_inp = intOnlytoscore(xaction)
		...
		while True :
			...
			if brun_c > player_score :
				break
			busr_inpb = intOnlytoscore(yaction)
	...
	def bowlingfirst () :
		...
		# while True :
		# 	#bover = input("Enter the overs u need to play = ",)
		# 	bover =  random.randint(10,20)
		# 	if bover.isdigit() :
		# 		bover = int(bover)
		# 		break
		# 	else :
	...
	if usr_in == coin_random :
		...
		while True :
			usr_selection = random.choice(play_selection)
